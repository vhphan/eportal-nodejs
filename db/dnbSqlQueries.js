...
FROM stats."NRCELLFDDDailyNetwork" t1
         FULL OUTER JOIN stats."PRBCQIRSSITADailyNetwork" t2 USING ("DATE_ID") ORDER BY t1."DATE_ID" ;`,
...
;
    ...
    group by time, "ERBS"
     ...
     t2 AS (
         SELECT "DATE_ID"                                            as "time",
                ...
                SUM("Sum(pmRadioRaAttTaDistr) for 9")                as "Sum(pmRadioRaAttTaDistr) for 9"
         FROM dnb.stats."DataTable"
         group by time, "NR_NAME", "NRCellDU"
     ),
     t3 AS (
     SELECT "DATE_ID"                                  as "time",
            "NE_NAME",
    ...
    FROM t2
     inner join t1 on t1."ERBS"=t2."NR_NAME" and t1.time=t2.time
     inner join t3 on t1.time=t3.time and t2."NR_NAME"=t3."NE_NAME"
    WHERE t2."NRCellDU" = $1
    ORDER BY t1.time
    `
}
