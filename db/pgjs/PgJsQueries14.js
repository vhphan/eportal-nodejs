const {sqlDnb, getSql} = require('./PgJsBackend14');


const getRaster = (operator) => async (request, response) => {
    const sql = getSql(operator);
    const {raster} = request.params;
    const gridType = request.query['gridType'] || 'hex_100';

    const table = operator === 'dnb' ? `dnb.rasters.nr_avg_${gridType}` : `celcom.rasters.lte_avg_${gridType}`;
...
const getRasterPointsInGrid = async (request, response) => {
    ...
    hex;
    on;
    st_intersects(hex.geom, nr_raster.geometry);
    where;
    folder =${folder};
    `;
    response.status(200).json({
