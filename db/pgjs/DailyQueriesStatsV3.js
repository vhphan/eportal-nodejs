...
const regionDailyStatsNR = async (request, response) => {
    const results = await sql`
        ...
        WHERE t1."Region" <> 'All'
          ...
          and t1."DISTRICT" = 'All'
          and t1."Cluster_ID" = 'All'
        ORDER BY t1."Region", t1.date_id, t1.id
    `;
    return sendResults(request, response, results);
};
...
const clusterDailyStatsNR = async (request, response) => {
    ...
    const results = await sql`
        ...
        WHERE t1."Cluster_ID" <> 'All'
          AND t1."Cluster_ID" LIKE ${clusterId}
        ORDER BY t1."Cluster_ID", t1.date_id, t1.id;`;
    return sendResults(request, response, results);
}
...
const customCellListStatsLTE = async (request, response) => {
    ...
    const results = await sql`
        WITH counters1 AS (SELECT "date_id",
                                  ...
                                  -- </editor-fold>
                           FROM stats_v3."EUTRANCELLFDD" as dt1
                                    LEFT JOIN (SELECT date_id,
                                                      erbs,
                                                      eutrancellfdd,
                                                      pmflexerabestabsuccinit  as pmflexerabestabsuccinit_endc2to99,
                                                      pmflexerabestabsuccadded as pmflexerabestabsuccadded_endc2to99,
                                                      pmflexerabestabattinit   as pmflexerabestabattinit_endc2to99,
                                                      pmflexerabestabattadded  as pmflexerabestabattadded_endc2to99
                                               FROM stats_v3."EUTRANCELLFDD_FLEX"
                                               WHERE flex_filtername ilike 'Plmn%endc2to99') as dt2
                                              using (date_id, erbs, eutrancellfdd)
             ...
             counters2 AS (SELECT "date_id",
                                  ...
                                  -- </editor-fold>
                           FROM stats_v3."EUTRANCELLFDD_FLEX" as dt1
                           WHERE dt1.eutrancellfdd IN ${sql(cells)}
        ...
        FROM (SELECT erbs,
            ...
            FROM stats_v3."EUTRANCELLFDD_V" as d1
        ...
        FROM stats_v3."EUTRANCELLRELATION" as dt1
        WHERE dt1.eutrancellfdd IN ${sql(cells)}
        group by "date_id")
        SELECT counters1.date_id:: varchar(10) as "time",
       ...
       ((0.04 * (pmtainit2distr_0) + 0.155 * (pmtainit2distr_1) + 0.43 * (pmtainit2distr_2) + 0.82 * (pmtainit2distr_3) +
       ...
       100 * (pmhoexesucclteinterf || pmhoexeattlteinterf)                                                 AS "Interfreq HOSR"
        FROM counters1
            LEFT JOIN counters2
        ON (counters1.date_id = counters2.date_id)
            LEFT JOIN counters3 ON (counters1.date_id = counters3.date_id)
            LEFT JOIN counters4 ON (counters1.date_id = counters4.date_id)
        ORDER BY time
        ;
    `;
    return response.status(200).json(results);
